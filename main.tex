...
\subsection{Hardware Required}
You will need an ESP8266 Wifi Module, a Pixhawk6C, a Radio Controller, IR Markers, and of course, your drone. The drone used here is a F450 Phantom Case with SV motors, bridge with a ESC adapter and powered by a 4S battery. 
% [image here]
...
Power up the Pixhawk, go to your GCS, and find the network. (By default, it should be whatever \footnote{If you are using the drone after the 2025 cohort, it is named as Inspector/Surveyour with password being inspector/surveyor correspondingly}. Enter the website and go to an browswer and type in the ip address. A screen of wifi module should pop up. Go into the setup parameters and change your name correspondingly. noting the host port as this is your way to communicating with your Wifi). Save your changes and reboot. You now have a link between the ESP module and your GCS. You now need to configure the Pixhawk so that it is compatable with the wifi module.  

\subsection{MissionPlanner Configuration}
Connect the usb-c port of the Pixhawk6C to a copmuter. Ensure the cable allows data transfer. Download MissionPlanner, open missionplanner:

Head towards parameter

Go to MissionPlanner and set the following parameters: 
...
Heaad towards setup and mandatory hardware, go to Mode switch, here you can switch between flight modes as required. Noting that altitude, loiter, are flight modes that uses GPS data from VICON.  

\subsection{IR Markers}
Mount the IR markers in an assymptical way and have at least 4+ per drone. Noting that if you are flying more than 1 drone, your assortment of IR markers must be geometrically different, otherwise the Vicon server may mistake the object (which has happened to me before and caused a crash.) An example of which can be shown below 

[figure]


\subsection{Mavproxy Telemetry}
Now, once IR markers are palced and Ardupilot configured, test if you recieve telemetry from your GCS. Go to the GCS and type in the following: 

If it is 14551 then you type in 14551 


You should have telemetry like this: 

\subsection{GCS to Vicon }
Place your drone inside the flight arena and return to your GCS. 


Refer back to section on how to define your drone as a new object. 

Go to file and change the name of your object to whatever you it is on the Vicon Host PC  

Return to MAVproxy console, Enter module load vicon inside the Mavproxy and vicon set and vicon start.



Open the console, you should see the the xyz and Euler angles of your drone. 
