\section{Pathplanning - Nura Efendic}
\label{sec:Pathplanning}

\subsection{Overview}
From the above, we have established how to connect to the MAVLink stream, how to arm the vehicle, how to takeoff, and how to send local position commands. This section will focus on how you can customize pathplanning and imperatively intergration towards your own designed algorithm. The content of this section is based on the MAVLink documentation for guided mode,which enables external programs to command the drone's motion in real time.

Instead of using global coordinates like above, local position control can also be used. Tj common approach for precise control, especially in indoor or GPS-denied environments, is to use \textbf{local position control} rather than global GPS coordinates. This is achieved using the \texttt{LOCAL\_NED} reference frame and corresponding MAVLink messages.
